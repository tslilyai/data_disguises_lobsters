...
for u in nusers:
    ...
    xs = list(edit_results_baseline[u][0].keys())
    order = np.argsort(xs)
    xs = np.array(xs)[order]
    ys = [statistics.mean(x) for x in edit_results_baseline[u][0].values()]
    ys = np.array(ys)[order]
    axes_flat[0].plot(xs, ys)

    xs = list(normal_edit_results[u].keys())
    order = np.argsort(xs)
    xs = np.array(xs)[order]
    ys = [statistics.mean(x) for x in normal_edit_results[u].values()]
    ys = np.array(ys)[order]
    axes_flat[1].plot(xs, ys)
    ...
    for i in range(len(props)):
        ...
        ys = np.array(ys)[order]
        axes_flat[2].plot(xs, ys, label='{} Disguisers'.format(props[i]*u))
    ...
    axes_flat[0].set_title("Baseline Latency")
    axes_flat[1].set_title("Latency with Normal Disguising")
    axes_flat[2].set_title("Latency with Worst-Case Disguising")
