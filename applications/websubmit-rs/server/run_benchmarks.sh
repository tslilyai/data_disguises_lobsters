...
for l in 20 40; do
	...
	RUST_LOG=error perflock ../../../target/release/websubmit-server \
        -i myclass --config sample-config.toml --benchmark true --prime true --nusers $u --nlec $l --nqs 4 --baseline true &> \
            output/${l}lec_${u}users_baseline.out
	echo "Ran baseline test for $l lecture and $u users"

	RUST_LOG=error perflock ../../../target/release/websubmit-server \
        -i myclass --config sample-config.toml --benchmark true --prime true --nusers $u --nlec $l --nqs 4 --baseline false &> \
