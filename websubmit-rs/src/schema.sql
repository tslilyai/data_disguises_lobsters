...
QUERY submitted: SELECT answers.`user`, answers.lec, COUNT(answers.q) AS num_answered FROM answers GROUP BY answers.`user`, answers.lec;
QUERY lec_qcount: SELECT questions.lec, COUNT(questions.q) AS qcount FROM questions GROUP BY questions.lec;
