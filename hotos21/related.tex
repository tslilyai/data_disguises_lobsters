...
%
In contrast, data masking does not keep PII in the application, %unless specified by the mask,
or change the application schema.
%
Sypse also sits between application's logic and data, and transforms the entity graph.
...
writing these specifications is challenging, and propose a menu of data mask transformations to
...
In contrast, data masking requires no modifications to application schemas or shared application data.
%
Data masks for user unsubscription can potentially be used to extract a user's shard from the
...
Data masks support privacy transformations beyond simply deleting PII, and can draw upon
...
data.  Data masking can draw from DELF's insights of how to address errors in developer
...
application data stores. However, DELF focuses only on data deletion at Facebook, while data masking
encapsulates this as well as other privacy transformations that require \eg decorrelating edges.
...
Data masking's goal is orthogonal: these frameworks do not transform the actual data stored by
...
In contrast, data masking helps developers specify and automate better privacy transformations
...
differential privacy~\cite{dataminingmodels, differential}, complement to data masking: for
