...
Prior work propose designs to make it easier for applications to implement data deletion policies.

Sypse~\cite{sypse} pseudonymizes user data and partitions personally identifying information (PII)
from non-identifying data. 
%and allowing the application to use the latter for legal or processing purposes. 
%
In contrast, data disguing does not keep PII in the application, %unless specified by the mask,
or change the application schema.
%
Sypse also sits between application's logic and data, and transforms the object graph.
%breaks foreign key relationships and objects grouped by user. 
However, Sypse assumes a specification of how to transform the graph, whereas we observe that
writing these specifications is challenging, and propose a menu of data disguise transformations to
allow developers to capture application-specific requirements. 
%We concretely define the application entity graph and abstractions that allow developers to reason
...
applications, splitting application data into user shards. 
%Applications can then erase a user by
%removing the user's shard. 
In contrast, data disguising requires no modifications to application schemas or shared application data.
%
Diguises for user unsubscription can potentially be used to extract a user's shard from the
application, while performing the necessary privacy transformations to the rest of application data.

%---------------------------
SchengenDB~\cite{schengendb} performs fine-grained tracking of PII and restricts query access to
PII.
%using purpose-based access control. SchengenDB performs fine-grained tracking of PII copies and data derived from PII,
%going beyond correlations visible in the entity relation graph of foreign key relationships.
%SchengenDB deletes records tracked with the owner when the owner unsubscribes.  
Data diguising supports privacy transformations beyond simply deleting PII, and can draw upon
SchengenDB's fine-grained tracking to add additional support for derived data not visible in the
object graph. 
...
data.  Data disguising can draw from DELF's insights of how to address errors in developer
specifications, which we currently trust and leave out of scope. 
%
DELF similarly takes as input annotations on application edge and object types encoded by
application data stores. However, DELF focuses only on data deletion at Facebook, while data
disguising encapsulates this as well as other privacy transformations that require \eg decorrelating edges.
%for a wider range of privacy policies
%, \ie supporting the Facebook data deletion privacy policy.  
...
general information flow control approaches~\cite{static, jeeves, jif, hails, ifdb} that check
that applications abide by data disclosure policies.
...
Data disguising's goal is orthogonal: these frameworks do not transform the actual data stored by
the applications, and instead only control what becomes visible at the front end.
%Data masking can additionally automate complex privacy transformations for authorized or multiverse views.
...
ownership paradigms in which applications compute on decentralized, per-user datastores.
%agree upon a standard format for data storage, and 
%hosted by a trusted third-party provider or the user. 
Application logic runs on user clients, %(\eg JavaScript~\cite{solid, blockstack, bstore})
and queries individual datastores to access user data. 
However, this results in greatly reduced performance, 
burdens users with long-term data maintenance and storage,
removes the flexibilty of application-specific SQL databases,
and breaks the current application revenue model.
%, and lose the flexibility and performance that results from specialized schemas and data storage.  

In contrast, data disguising helps developers specify and automate better privacy transformations
...
differential privacy~\cite{dataminingmodels, differential}, complement to data disguising: for
analytical queries, techniques like PINQ~\cite{pinq} suggest how to provide differentially private guarantees.
