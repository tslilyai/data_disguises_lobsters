...
Frameworks such as Jeeves~\cite{jeeves} aid developers in enforcing privacy policies that specify
how sensitive values should be disclosed by a program. Like \sys, the framework also relies on developers to
specify appropriate policies, and simply enforces them. However, \sys works in the domain of
database data transformations and applies mask policies only when invoked, whereas Jeeves does not modify
data and instead monitors program behavior at runtime to track where and how data is disclosed.
\lyt{I'm not sure that this is that related, but it
came up in PDOS...}
