...
%-------------------------------------------------------------------------------
\label{sec:related}
\lyt{Should we also cite the GDPR-related DB papers? \eg Malte's paper regarding user shards, SchengenDB,
etc?}

\paragraph{Decoupling User Data.}
Prior work~\cite{solid, amber, w5, blockstack, bstore} has proposed user-centric data ownership
paradigms which decouple user data from applications. Applications agree
...
\paragraph{Pseudonymization of User Data.}
...
purposes. Similar to our approach to \name, Sypse breaks up foreign key relationships and objects
grouped by user; however, Sypse assumes a specification of which objects to anonymize and which
connections to break, whereas we propose abstractions to express such policies that capture
application-specific behavior and requirements. We concretely define the application entity graph
and abstractions that allow developers to reason about it, whereas Sypse leaves this as future work.
%

Sypse's proposed synthetic data generation is similar to our idea of ghost entities, but their data
...
Instead of constantly maintaining data partitions, our approach to \name encrypts a record of the changes made
(often to PII) during unsubscription, incurring little overhead during normal execution. A future
implementation may preemptively create ghost data and result in a system design vaguely similar to
Sypse, but this implementation strategy is not fundamental.
