...
well as \texttt{DISGUISE [disguise\_id] [target]} and \texttt{REVERSE DISGUISE [disguise\_id] [target]} queries.  \sys provides
data disguising for applications that use SQL relational databases, where objects are
tables rows and edges are foreign key relationships.
%
This restriction is not fundamental: \sys could additionally support abstract,
non-table objects such as ``country,'' or work on top of a graph or blob datastore instead.
