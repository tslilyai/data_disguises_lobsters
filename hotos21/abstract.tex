\begin{abstract}
Web applications face increasing legal and user demands to transparently manage user data,
whether through supporting user's right to be forgotten, removing unnecessary or old user data,
or moderating user content.
%
Meeting these demands to \emph{mask} data requires applications to
remove or modify data, while still retaining data required for legal purposes or
to preserve application utility.
%
Because such transformations are necessarily application-specific and
require handling complex data correlations, today's applications often forgo these
transformations, or implement ad-hoc transformations with unclear guarantees.
\ms{sets us up to specify what guarantees we make}
%
\ms{propose to rewrite following paragraph to make it more HotOS-y:
We propose \emph{data masking}, a systematic approach that helps developers
generate concrete privacy transformations for database-backed web applications from
a high-level specification.
%
%
Data masking tools address use cases that require onerous manual labor today, such
as GDPR compliance, user unsubscription, or data deidentification.
%
Importantly, data masks enable reversible transformations that permit returning to
an identity-revealing state at any time.
%
