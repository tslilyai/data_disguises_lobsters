...
%
%whether through supporting user's right to be forgotten, or removing unnecessary or old user data.
%
But it is hard to be a conscientious developer: privacy-enhancing database transformations may require deleting, anonymizing, and decorrelating users' data, while maintaining data required for legal purposes or to preserve application utility.
%
Meeting these requirements grows only more challenging as privacy transformations become more complex, and today's applications support only simple, coarse-grained, and ad-hoc privacy transformations.
...
We propose \emph{data masking}, a systematic approach that helps developers generate privacy transformations for database-backed web applications from a high-level specification.
%
...
enable reversible transformations, facilitating user return.
%
Furthermore, data masks reduce developer burden, as developers must no longer manually implement privacy transformations, and enable key new use cases, such as user-specific privacy modes.
%
A prototype data masking tool demonstrates that our approach is practical.
