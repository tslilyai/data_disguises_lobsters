...
data disguising.  \sys's performance overheads stem from (1) adding a layer of indirection between
application and database, and intercepting and parsing queries; (2) keeping an (in-memory) object graph to easily traverse during disguise
application and reversal; and (3) creating and updating guises as necessary to apply disguises.
We implemented several variants of \sys to explore design performance tradeoffs. 
...
and caching query results. All transformations and guise creation occur at disguise application. In
...
Disguising needs only to expose the shadow copy and its guises in the MV, and remove
objects that should be deleted.
%
This design achieves good read performance---queries only need to read from MVs---at the cost of
high memory consumption; however, updates become many times more expensive because they must
update guises in the shadow copy.
