...
\section{Evaluation}
%-------------------------------------------------------------------------------
We evaluate \proto on two fronts: (1) does \proto's unsubscription policy abstractions allow the
developer to sufficiently and easily express how data should be retained and de-identified?; and (2) 
can \proto support unsubscription and resubscription with low performance overhead?

\subsection{Writing Practical Unsubscription Policies}
...
\begin{figure*}[t!]
    ...
    \textbf{(a)} \proto decorrelates user-review and user-paper edges, and retains
    review-paper edges, generating ghost users randomly, but cloning each review and paper. \\
    \textbf{(b)} \proto decorrelates user-conflict edges and sensitive conflict-paper edges. The ghost
    paper associated with non-sensitive conflicts is a clone of the original; \proto generates a
...
We use \proto to write an unsubscription policy for HotCRP, demonstrating that \proto's abstractions
...
%Figure~\ref{fig:hotcrp} illustrates how unsubscription changes a simplified HotCRP entity graph
%with users, papers, reviews, and paper conflicts.
The developer first specifies that the top-level entity to decorrelate is a user.

\lyt{For brevity, I've removed a lot of details/corner cases of the policy.}

\paragraph{Edge Policies.} 
Next, the developer considers all edge types, and whether edges may
reveal identifying information about the user.
All direct correlations between a user and other entities 
may leak the user's identity. The developer therefore specifies edge policies with a
sensitivity threshold of 0 for edges with the user as a direct parent. 
Because HotCRP should retain the paper and review data, and merely decorrelate these data from their
author's identity, the developer specifies that \proto should decorrelate these edges to meet the
threshold: \proto will replace any edges from papers or reviews to the unsubscribing user with an
edge to a ghost user profile. Figure~\ref{fig:hotcrp}a shows how \proto decorrelates a user from
...
When a user unsubscribes, the developer must also ensure that edges from these sensitive paper
conflicts their parent papers cannot leak identifying information.
%the edge policies described above ensure that the user is decorrelated from
...
These choices demonstrate the tradeoff between
...
%user. \proto decorrelates these edges until the threshold is met; all ghost tag attributes are
%generated using a clone-one ghosting policy.  Figure~\ref{fig:hotcrp}c shows how \proto partially
...
The final group of edge policies ensure that review and paper artifacts remain present and correctly
linked with other application entities: (subscribed) reviewers should still see the correct paper
for each of their reviews, (subscribed) authors should see the correct reviews for their papers, and
review and paper metadata should remain correctly associated. The developer specifies a sensitivity
threshold of 1 for these edge types, indicating that \proto should leave these edges untouched.

\paragraph{Generating Ghost Entities.}
The developer also specifies how \proto generates ghost entities. This involves determining
...
semantics, the relevant edges should be deleted instead of decorrelated.  For simplicity,
Figure~\ref{fig:hotcrp} shows entity-granularity ghosting policies rather than per-attribute
ghosting policies, and does not display the generation method (\eg random, default, etc.).
...
%; however, any cloned attributes should not reveal any identifying information.
...
paper attributes do not reveal identifying information, and that all edges to sensitive parents will
be decorrelated.

%In particular, it is safe to clone edges to user parents because the developer knows that \proto will
...
ghosts. As with paper entities, it is safe to clone edges to user and paper parents because any
...
\subsection{\proto Performance}
We write a prototype implementation for \proto in Rust, which acts as an in-memory shim layer on top
of a MySQL database, allowing \sys to intercept and process queries sent by the application frontend. 
...
\sys is build in 5K LoC of Rust, and adds \texttt{UNSUBSCRIBE
[user]} and \texttt{RESUBSCRIBE [user] WITH [user\_data]} calls as supported database queries.
...
\iffalse
We evaluate \proto on two metrics: (1) can desired decorrelation policies be easily specified using the
...
range of privacy policies using \proto with low developer effort with low overhead.
...
With \proto, we can express better, more granular privacy policies that would otherwise be difficult to
...
with \texttt{NoDecorrelation:Retain} policies, which means that \proto does, by default, no work upon
...
\proto allows PrestaShop to go beyond pseudonymity, creating individual (ghost) accounts for each
...
\proto also can help PrestaShop keep attributes such as the country or language of a user for
...
\includegraphics[width=.32\textwidth]{img/shim_only}
    \label{fig:results}
    \caption{Comparison of \proto's latency during normal execution.}
\end{figure*}

\paragraph{Experimental Setup.}
The performance experiments reported are run on Intel Xeon E5-2660 v3 CPUs, with a
single-threaded client and \proto's shim layer each pinned to a single core. \proto runs on top of 
MariaDB (v10.4.13). 
...
\begin{enumerate}
    ...
    \item \texttt{ShimParsing}: queries are intercepted the shim layer, parsed into a
        ...
        client. 
    \item \texttt{\proto}: queries are intercepted by the shim layer and parsed into a SQL AST. \proto
        ...
        \proto then returns results to the client.
\end{enumerate}
...
\texttt{\proto} to define the cost of adding decorrelation and recorrelation support.  \texttt{Shim}
provides an upper bound for \proto's (single-threaded) performance, as the costs of query parsing
and decorrelation are not necessarily fundamental.

\paragraph{Lobste.rs Performance}
\proto utilizes the trawler workload~\cite{trawler} for Lobsters, which emulates production
...
latencies, (3) the query multiplication per application query performed by \proto, and (4) the
storage cost (in-memory and on-disk).

The first benchmark measures \proto's performance during \emph{normal execution}, namely when only
read and update actions performed by Lobsters are executed. 
The second benchmark measures \proto's performance with users unsubscribing 10\% of the time
...
TODO, \texttt{ShimParse} takes 316s; and \texttt{\proto} takes 185s.
...
takes 284s; and \texttt{\proto} takes TODO.
\lyt{TODO memory usage}
\fi
