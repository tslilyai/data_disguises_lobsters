...
complexity and difficulty as writing relational schemas. In particular, writing a disguise should
occur only once, and require the same application expertise as specifying schemas. Because disguises
require at most one tranformation for each object type and edge type, the complexity is limited by
the number of entity types and relations in the application schema.  Table~\ref{tab:loc} shows the
statistics for each disguise. 

We next describe disguises for HotCRP unsubscription in more detail.  HotCRP objects include users
(reviewers and authors), papers, paper reviews, and paper conflicts.  The main concern upon user
unsubscription is that users' reviews and authored papers should be as unlinkable to their identity
as possible.  Someone viewing conference data should have access to only the papers or reviews they
are allowed to view, and not be able to reassociate these papers or reviews with an unsubscribed
user.
...
\label{sec:perf}
We face many challenges to achieve practical performance in \sys. \sys incurs performance overheads
from introducing a shim layer between the application and its database, and space and performance
overheads when applying disguises and creating guises. \sys also keeps an in-memory constructed
object graph to easily traverse during disguise application and reversal, adding to normal execution
runtime costs.

We implemented several variants of \sys. The first only tracks the graph of objects during normal
execution, and performs all transformations and guise creation when the disguise is invoked. Normal
execution incurs roughly a 10\% overhead, mostly from query interception and parsing, but requires
enough memory to store the object graph. Disguising requires potentially as much work as the number
of objects related to the target in the graph, to \eg create guises for each post created by the
user; and guise creation can multiply one object into several, costing both memory and disk space.

The second variant amortizes the cost of disguising across normal execution, adding guises to the
database during normal execution when objects are inserted or modified.  Pre-created guises are
hidden by transparently exposing materialized views with only objects to the application. Disguising
reveals guises in the materialized views, rather than creating them all at once. This reduces the
overhead of applying the disguise, but multiplies the number of database queries for object updates
during normal execution by potentially the number of guises associated with that object, and
consumes additional disk space for pre-created guises even during normal execution.

The performance and space tradeoffs in these variants and other implementation designs, such as
caching disguises to be reapplied after reversal, and spreading out the cost of disguising by
supporting \eg only eventually consistent disguises, are exciting directions to explore.
