...
\end{tabular}
    \caption{Statistics for account deletion disguises in Lobsters and HotCRP.
...
\begin{figure}[t!]
    ...
    \caption{Work required to disguise and reveal a target user, depending on the number of
...
the number of edges connected to the target. As shown in Figure~\ref{fig:latencies}, the amount of
work to create guises and modify objects and edges when disguising (and
revealing) increases linearly with the number of objects correlated with the target.
...
This indicates a need for privacy-aware database designs, which
spread the cost of disguising across normal execution.
%
