...
the other two requirements by evaluating the ease of writing and expressiveness of unsubscription
policies in HotCRP, a popular conference review management application, and Lobsters, an open-source
news feed application. \lyt{and PrestaShop?}
...
unsubscription policy specified in \sys requires \lyt{TODO} lines of code.

We first consider what information must to be removed to unsubscribe the user.  Clearly, all direct
correlations between a user and their papers, reviews, and paper conflicts may leak the user's
identity, and must be broken: we assign edge policies with a sensitivity threshold of 0 to edges
with the user as a direct parent. Because HotCRP should retain the paper and review data for other
users, these edge policies decorrelate, rather than delete, direct edges from user to papers and reviews: 
\sys will replace these edges with an edge to a generated ghost user profile (Figure~\ref{fig:hotcrp}a).
...
As observed earlier, breaking direct correlations to the user is not sufficient. HotCRP has paper
conflict entities, which link to both a user and a paper. Paper conflicts identify users who either
coauthor the paper, or cannot review the paper because of a conflict of interest. If HotCRP retains
paper entities, we cannot simply decorrelate a user from their paper conflicts, because the paper's
other conflicts can identify even anonymized paper conflict users. Instead, we specify an edge
policy for paper-to-paper conflict edge types that \emph{deletes} all edges between sensitive paper
conflicts and papers (Figure~\ref{fig:hotcrp}b). 

Although spurious conflicts may arise as a result (\eg no coauthor is affiliated with NYU, but
reviewers from NYU have conflicts), this does not interfere with HotCRP semantics. This may still
leak identifying information, but less so than if the sensitive conflict were retained: spurious
conflicts may not always arise (\eg subscribed coauthors are also from NYU), and PC chairs viewing
conflicts will not know \emph{how many} paper conflicts have been removed (how many users have
unsubscribed). To ensure complete removal of all identifying information, we would need to delete
\emph{all} the paper's conflicts, which would incorrectly allow users with reviewer conflicts to
review a paper and break HotCRP semantics.
...
We also specify how \sys generates ghost entities, determing which entity
...
For clarify, Figure~\ref{fig:hotcrp} shows simplified, entity-granularity ghosting policies rather than
per-attribute ghosting policies.
We generates ghost users' attributes, giving all ghosts a default value of \texttt{disabled=true}
