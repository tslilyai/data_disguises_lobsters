...
let entity_transforms = vec![
    GhostGenerationPolicy {
        entity_type: "ContactId",
        attribute_transforms: vec![
            ("username", Generate(Random)),
            ("email", Generate(Default(nobody@gmail.com))),
            ("disabled", Generate(Default(true))),
            ...
        ],
    },
    ...
...
let edge_transforms = vec![
  ...
  EdgeTransform {
    ...
    parent_child_transform: Retain,
    child_parent_transform: Delete,
  },
  ...
];

let hotcrp_unsub_mask_policy = MaskPolicy {
  unsub_entity_type: "ContactId",
  entity_transforms: entity_transforms, 
  edge_transforms: edge_transforms,
};
\end{lstlisting}
    \label{fig:hotcrp_policy}
    \caption{Excerpt of the HotCRP mask policy for unsubscription}
\end{figure}
...
let edge_transforms = vec![
  ...
  EdgeTransform {
    ...
    column_name: "tag_id",
    parent_child_transform: Decorrelate,
    child_parent_transform: Decorrelate_Sensitive(0.25),
  },
  ...
];
...
The policy excerpt in Figure~\ref{fig:hotcrp_policy} shows the specified edge transformations for paper
conflicts to user (\texttt{ContactId}) and paper entities.
...
(excerpt shown in Figure~\ref{fig:lobster_policy}).
