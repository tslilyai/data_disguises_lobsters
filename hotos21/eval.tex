...
conflicts and papers (Figure~\ref{fig:hotcrp}b). Although spurious conflicts may arise as a result
(\eg no coauthor is affiliated with NYU, but reviewers from NYU have conflicts), this does not
interfere with HotCRP semantics. Note that this still may leak identifying information, but less so
than before: spurious conflicts may not arise (\eg subscribed coauthors are also from NYU), and even
if they do, PC chairs viewing conflicts will not know \emph{how many} paper conflicts have been
removed (how many users have unsubscribed). In order to completely remove all identifying
information, we must delete \emph{all} paper conflicts of the paper; however, this 
allows reviewers with conflicts to review a paper, failing to satisfy HotCRP semantics.
...
\textbf{Lobsters} has a total of 20 entities, and has a \lyt{TODO} LoC unsubcription policy.
...
placeholder. In addition, we desensitize tags so the
...
\lyt{TODO: write in terms of sensitivity threshold, policy abstractions}
