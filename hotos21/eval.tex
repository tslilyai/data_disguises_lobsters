...
\begin{table}[t!]
    ...
    \midrule
    Lobsters-Unsub & 19 & 20 & 318 & 220 \\
    HotCRP-Coarse & 25 & 50 & 352 & 378 \\
    HotCRP-Granular & 25 & 50 & 352 & 407 \\
\end{tabular}
    \caption{Unsubscription disguise statistics for Lobsters and HotCRP.  
...
should write a disguise only once. Because disguises require at most one tranformation for
...
Lobsters-Unsub and HotCRP-Coarse implement the current unsubscription policies in the respective
applications.
...
%
The HotCRP-Granular disguise specifies a HotCRP unsubscription policy that balances useful data
...
\begin{figure}[t!]
\centering
    \includegraphics[width=.5\textwidth]{img/latencies}
    \caption{Disguising and revealing latency vs. the number of objects belonging to
    the target user.}
    \label{fig:latencies}
    \vspace{-\baselineskip}
\end{figure}

Our experience implementing \sys exposed several performance challenges, in particular highlighting
the tradeoff between good performance during normal execution, and low-latency disguise application.
The right balance depends on the rate of disguising, which may range from rare (as in today's
applications), to quite frequent in a privacy-supporting world where users freely disguise
and reveal themselves, or where data automatically ages out.
...
Our first \sys variant adds minimal overhead to normal execution, only intercepting queries
and tracking an in-memory the object graph (roughly 9\% throughput decrease). 
%Adding the shim layer decreases throughput by roughly 4\%,
%and query parsing and graph tracking reduces throughput another 5\%.
%, which can be improved by using prepared queries and caching query results.  \lyt{These numbers
%come from lobsters running at 0.5 scale, w/0\% unsub}
Disguise application in the worst case requires creating as many guises as
the number of edges connected to the target. As shown in Figure~\ref{fig:latencies}, disguising (and
revealing) latency increases linearly with the number of objects correlated with the target.
%; revealing is cheaper because it requires removing many guises and restoring a few objects,
%instead of generating many guises and removing a few objects~\lyt{this might be too much detail..}.

We are also investigating other designs that amortize the cost of
disguising across normal execution.
%
One possibility keeps a ``shadow'' copy of the database that
reflects the disguised state, with guises already created and edges rewritten.
%
...
high memory consumption. However, normal execution still incurs the cost of query interception, and
write queries may also update multiple guises in the shadow copy.
%, leading to query multiplication.
Pre-created guises also consume space during normal execution, even if disguises are never applied. 
%
Other possibilities include making disguises eventually consistent; updating the shadow
copy asynchronously; and caching disguises for users who are disguised (and revealed) most
often.
