...
\begin{figure}[t!]
    ...
    paper clones the original; the other is a default placeholder paper. %Paper conflicts are cloned.
    }
    \label{fig:hotcrp}
\end{figure}

We evaluate whether our approach meets the requirements of \name in practice,  
allowing developers to sufficiently and easily express how data should be retained and
de-identified, and supporting resubscription without burdening the developer or user. 
%and (2) %does \sys support unsubscription and resubscription with low performance overhead?
Our approach satisfies automatic resubscription, handling data encryption for the application and
user without requiring the user to manage private keys or data storage. We show that \sys can meet
the other two requirements by evaluating the ease of writing and expressiveness of a mask policy for
user unsubscription in HotCRP, a popular conference review management application, and Lobsters, an
open-source news feed application. \lyt{and PrestaShop?}
...
mask policy for unsubscription specified in \sys requires \lyt{TODO} lines of code.

The developer first decides what information should be modified during unsubscription. 
This includes all direct correlations between a user and their papers, reviews, and paper conflicts,
which may leak the user's identity: the developer chooses edge transformations with a sensitivity
threshold of 0 to all edge types with the user as a direct parent. 
Because the developer wants to retain paper and review data for other users, these edge policies
decorrelate, rather than delete, direct edges from user to papers and reviews: \sys will replace
these edges with an edge to a generated ghost user profile (Figure~\ref{fig:hotcrp}a).
...
As observed earlier, HotCRP may also want to handle non-user edges such as edges between paper
conflict entities and papers. Paper conflicts identify users who either coauthor the paper, or
cannot review the paper because of a conflict of interest. If HotCRP retains paper entities, the
paper's other conflicts can identify ghost users on the paper. The developer thus specifies an edge
transformation for paper-to-paper conflict edge types that \emph{deletes} all edges between
sensitive paper conflicts and papers (Figure~\ref{fig:hotcrp}b). 

The developer must ensure that this edge transformation does not break HotCRP's invariants. Deleting edges
between sensitive paper conflicts and papers can lead to spurious conflicts (\eg no coauthor is
affiliated with NYU, but reviewers from NYU have conflicts), but does not break paper access 
invariants in HotCRP, a key aspect of HotCRP's correctness. 
We note that a transformation resulting in a more complete removal of identifying correlations would
to delete \emph{all} the paper's conflicts. However, this would incorrectly allow users with reviewer
conflicts to review a paper and break HotCRP semantics.

For all other edge types, the developer specifies a sensitivity threshold of 1. This ensures that review
and paper artifacts remain correctly linked: (subscribed) reviewers still see the
correct paper for their reviews, and (subscribed) authors see the correct reviews for
their papers.
...
The developer also chooses entity transformations to specify how \sys generates ghost entities.
The developer deterimes which entity attributes reveal identifying information (and should be generated); which attributes should be cloned to retain information necessary to the application; and how to generate attribute values in ways that satisfy HotCRP semantics. 
For clarify, Figure~\ref{fig:hotcrp} shows simplified, entity-granularity ghosting policies rather
than per-attribute ghosting policies.
Ghost users' attributes are generated with a default attribute value of \texttt{disabled=true}
to ensure ghosts never review papers. We clone all paper and review attributes once, retaining
paper and review information without creating duplicates. 
%and clones paper conflicts because no conflict
%value attributes are identifying.

\textbf{Lobsters} has a total of 20 entities, and has a \lyt{TODO} LoC unsubscription policy.
The policy replicates what the behavior of the current Lobsters deployment upon user
unsubscription, with the exception that we replace users with ghost users rather than a global
placeholder, and we desensitize tags so that sensitive stories comprise at most 25\% the number of
stories with a particular tag. This limits how much a particular tag topic can be used to identify a
user who often posts about that topic (e.g.,\ a researcher in visualization). 
All edges directly to the user have policies that decorrelate to a sensitivity threshold of 0; edges
between stories and tags have policies that decorrelate to a sensitivity threshold of 0.25 (with ghost tags generated
as some default tag); and edges between all other entities have policies that retain the edge
(sensitivity threshold of 1).
