...
\begin{figure}[t!]
    ...
    \textbf{(b)} \sys decorrelates user-to-conflict edges and conflict-to-paper edges. One ghost
    paper clones the original; the other is a default placeholder paper. %Paper conflicts are cloned.
    }
    \label{fig:hotcrp}
\end{figure}

We evaluate whether \sys meets the requirements of \name in practice,  
...
\sys meets the requirement for automatic resubscription, handling
...
ease of writing and expressiveness of unsubscription policies in HotCRP, a popular conference review management
application. \lyt{and Lobsters and/or PrestaShop?}

%We use \sys to write an unsubscription policy for HotCRP, demonstrating that \sys's abstractions
%are indeed expressive enough to handle realistic and subtle data policies. 
%This example illustrates the workflow and different application-specific choices developers must consider when
%using \sys. 
%
...
For edges with the user as a direct parent, the developer first specifies edge policies with a
sensitivity threshold of 0: all direct correlations between a user and other entities may leak the
user's identity. Because HotCRP should retain the paper and review data,
...
As observed earlier, edges not directly connected to the user also reveal identifying information:
collaborators or reviewers with conflicts on a paper may reveal a ghost author's identity. 
...
When a user unsubscribes, the developer must ensure that edges from sensitive paper conflicts to
papers cannot leak identifying information. To do so, the developer specifies that \sys should
decorrelate all sensitive paper conflicts from the paper so each sensitive paper conflict is
reassociated with a ghost paper parent (Figure~\ref{fig:hotcrp}b). The developer determines that
HotCRP can allow any spurious conflicts that may arise as a result.
...
Our prototype consists of 5K LoC of Rust, and supports SQL queries as well as \texttt{UNSUBSCRIBE
