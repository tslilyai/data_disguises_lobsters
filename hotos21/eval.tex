...
\fi
    ...
    \caption{State before and after applying the HotCRP-Granular mask for unsubscription. For
    clarity, we show only relevant parts of the entity graph and mask specification, and use entity-granularity ghosting
...
\begin{table}[th!]
    ...
    \footnotesize
    \begin{tabular}{@{}ccccc@{}}
        \multirow{2}{*}{\textbf{App}} & \textbf{\#Entities} & \textbf{\#Edge} & \textbf{Schema} &
        \textbf{Mask} \\
...
\end{tabular}
    \caption{Unsubscription mask statistics for Lobsters and HotCRP. Lobsters-Decor and
    HotCRP-Coarse
    masks implement the current unsubscription policies in the respective applications: Lobsters
    retains and reassigns user contributions to an anonymous user, and HotCRP deletes
    direct correlations to the user.}
\label{tab:loc}
\end{table}
We evaluate whether \sys enables developers to easily express and automate practical data masks by
writing mask policies for user unsubscription in Lobsters, an open-source news feed application, and
HotCRP, a popular conference review management application. Table~\ref{tab:loc} shows the statistics
for each mask; we describe our HotCRP policy in more detail below.

HotCRP entities include users (reviewers and authors), papers, paper reviews, and paper conflicts.
%There are a total of 25 entities, with 50 edge types between entities. 
...
The HotCRP-Coarse mask specifies the current HotCRP unsubscription policy, which simply deletes
...
edges.  
%
All sensitive entities are either removed or retained, and no extra ghost entities are
generated.  HotCRP-Coarse there does not explicitly specify any entity transformations (\sys by
default will copy retained entity content).
%
Each of the 50 edge transformation specifications requires 6LoC, and the remaining 78 LoC of the mask
definition come from defining \sys mask data structures for a total of 378 LoC. 

The HotCRP-Granular mask specifies a HotCRP unsubscription policy that balances data retention for
...
Because HotCRP-Granular generates additional ghost users to decorrelate reviews and papers from the
user, the mask specifies user entity transformation. 
HotCRP-Granular generates all of a ghost users' attributes with default or suitable random values
(\eg timestamps randomly generated within the past decade), in particular setting
\texttt{disabled=true} to ensure ghost users have no permissions and never review papers. 
This results in a slightly longer, 407 LoC mask specification.
