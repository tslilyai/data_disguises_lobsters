...
%The developer specifies that the top-level entity to decorrelate is a user.
...
%the edge policies described above ensure that the user is decorrelated from their paper conflicts.
%However, the developer must also ensure that edges from these sensitive paper conflicts their
%parent papers cannot leak identifying information.
...
Finally, the developer specifies a sensitivity threshold of 1 for all other edge types, ensuring that review
...
chooses to clone all paper attributes in one ghost, and generate default values for other ghosts.
