...
There are a total of 25 entities, with 50 edge types between entities. The main concern upon user
unsubscription is that users' reviews and authored papers should be as unlinkable to their identity
as possible.  Someone viewing conference data should have access to only the papers or reviews
they are allowed to view, and not be able to reassociate these papers or reviews with an
unsubscribed user.

We first specify a data mask current HotCRP unsubscription policy, which simply deletes users'
reviews, paper conflicts, and other review information. The corresponding specification is 378 lines
long, and chooses delete edge transformations for edge types to the user, and retain edge
transformations for everything else.  Because all sensitive entities are removed, no entity
transformations need to be explicitly specified.

We then consider a more desirable HotCRP unsubscription policy, in which reviews and review
information remain present to be available for other users to use.  This policy can be specified in
as a 400\lyt{TODO}-line mask specification.

Instead of deleting direct correlations between a user and their papers, reviews, and paper
conflicts, we choose edge transformations that decorrelate these edge types (retaining paper and
review data). \sys replaces these edges with an ones to generated ghost user profiles
(Figure~\ref{fig:hotcrp}a).

As observed earlier, a group of paper conflicts on a paper can also identify unsubscribed users by
exposing their affiliations and recent collaborations. We choose an edge transformation for
paper-to-paper conflict edge types that deletes all edges from sensitive paper conflicts to
(non-sensitive) papers (Figure~\ref{fig:hotcrp}b).  Deleting these edges can lead to spurious
conflicts (\eg no coauthor is affiliated with NYU, but reviewers from NYU have conflicts), but, as
with the original HotCRP policy, does not break paper visibility or review assignment invariants
...
We generate all of a ghost users' attributes with default values, in particular setting
\texttt{disabled=true} to ensure ghosts never review papers. We copy all paper and review attributes once, retaining paper and review information without creating duplicates. 
