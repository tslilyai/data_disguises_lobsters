...
%
When Bob unsubscribes, his profile and contributions move to different,
...
\begin{enumerate}[nosep]
  ...
  \item application code is prepared to handle the different guises, which all have the
...
%
Developers specify how to transform attributes of the template
...
Consider disguising Bob when he unsubscribes from HotCRP.com.
%
...
%
To do so, the disguise generates suitable default or custom guise attribute values
(\eg last logins randomly generated within the review period).
%
In addition, the guises's attribute \texttt{disabled} is set to \texttt{true}, ensuring that
guises have no permissions and never review papers.
%
...
%
The disguise \texttt{Retain}s all other edges in HotCRP, ensuring that review and paper
...
%
Unlike the current real-world HotCRP.com unsubscription policy~\cite{hotcrp:privacy}, which
