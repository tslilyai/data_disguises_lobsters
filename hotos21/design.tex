...
\paragraph{(2) Generate new content.}
        ...
        To generate edge attributes (\eg new foreign key relationships), the data mask recursively
...
transformations from Section~\ref{sec:survey}. Table~\ref{tab:masks} shows the entity and edge transformations for masks
that express even the more complex privacy transformations.

\begin{table*}
    \footnotesize
\begin{tabular}{p{0.33\textwidth}|p{0.33\textwidth}|p{0.33\textwidth}}
\textbf{Prose Specification} & \textbf{Edge Transformations} & \textbf{Entity Transformations}\\
...
\begin{lstlisting}[language=Rust]
"User-Contrib":     Decorrelate
"Contrib-Property": Decorrelate(Filter($\lambda(p, c). p.user_id == contrib.user_id$)
\end{lstlisting}
    & 
\begin{lstlisting}[language=Rust]
"User":     Generate(Random)
...
\end{lstlisting}
        \\
Encrypt the user's profile and reassociate their contributions with an anonymous
...
\end{tabular}
\label{tab:masks}
\caption{Example mask specifications a variety of privacy transformations (only relevant
