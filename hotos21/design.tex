...
\begin{table*}[t!]
    ...
    \footnotesize
\begin{tabular}{@{}cccc@{}}
\textbf{Entity Transformation Specification} & \textbf{Template Entity} & \textbf{Ghost1} & \textbf{Ghost2}
  \\ \cmidrule(r){1-4}
...
"User" : {
  ...
  "role": CloneOne+Generate(Default(None)),
}
\end{lstlisting}
    & 
\begin{lstlisting}[language=Rust]
...
\end{lstlisting}
\end{tabular}
    \caption{Generating two ghosts for a simplified HotCRP user entity.} 
    \label{tab:ghosting}
\end{table*}
...
the identifier attribute, and all other attributes are value attributes.
...
\begin{itemize}[nosep]
    ...
    \item \texttt{Generate:}
        ...
        the template value, or the encrypted template value).  To generate edge attributes (namely
        new foreign key relationships), \sys recursively generates a ghost parent entity, and
        creates a foreign key relationship to the new parent ghost identifier as the ghost edge
        attribute value.
...
how and when these to rewrite the structure of the entity graph using ghosts.
