...
%\end{enumerate}
%
At any given moment, then, an application's data object graph comprises a mix of
...
%
User objects have identifier \texttt{id}, and a reference to a tag object (\texttt{tag\_id}),
which forms an edge in the graph (a foreign key constraint to tag objects).
%
...
%
Developers choose how to create other guise attributes, selecting from the following:
%
\paragraph{(1) Copy object content.}
%
Guises of the same object all share the template's attribute values.
%
...
the original object's attribute value).
%
Figure~\ref{fig:guises} illustrates an example of random (\texttt{name}) and default
(\texttt{active}) generated value attributes.
%
%
Creating new guise edge attributes (\eg new foreign key relationships) requires, in order to
maintain referential integrity, creating a new guise for the referenced object. The data
disguise rewrites the edge to point to the new guise.
...
%
\lyt{I think it's inaccurate to say that we recursively \emph{disguise} the connected object, because this
would potentially imply that the algorithm rewrites an entire subtree of the graph that it doesn't
currently.}
%the data disguise
%recursively disguises the connected object, and rewrites the edge to point to the new guise.
%
...
\begin{enumerate}[nosep]
    ...
    %
    Creates one guise of the destination (using the destination object) for each of the
    sources; replaces each sources's edge to the destination with an edge to that unique guise.
    %
...
%
In particular, HotCRP users' \texttt{disabled} attribute is set to \texttt{false} for the guises,
