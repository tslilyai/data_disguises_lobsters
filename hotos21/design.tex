...
All entities such as reviews, papers, and review requests directly correlated with unsubscribed users may
leak the user's identity. The developers therefore should specify edge policies with a sensitivity
threshold of 0 for edges of type users-to-reviews, users-to-papers, and so on. Because HotCRP should
...
profile. The HotCRP developer further specifies that ghost user profiles are generated randomly,
with the exception that the template user's roles are cloned once, and the user account's disabled
status is cloned for all ghosts.
