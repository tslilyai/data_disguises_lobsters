...
%
We assume that developers use their domain knowledge to write correct and complete disguises\lyt{a
bit worried about ``complete'' here}, and that application code is prepared to handle the different
guises appropriately (\eg in displaying them), given that guises have the same structure as
application objects (\eg rows in existing application tables).

A data disguising tool takes the disguise specification and turns it into storage operations that
apply the transformation (disguise) or its reverse (reveal). The tool sits between the application
and its data, exposing additional queries to the application that allow it to invoke the disguise on
transformation-triggering events (\eg user account deletion, data timeout, etc.).
%
...
Data disguises consist of a set of chosen transformations on object types and edge types in the application
object graph.
...
To create a guise from a template object, developers specify how to transform attributes of a template
...
Consider disguising Bob when he deletes his HotCRP.com account.\lyt{Should we use HotCRP or
HotCRP.com? Seems like we switch between?}
%
