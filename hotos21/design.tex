...
only about the the \emph{types} of individual entities and the \emph{types} of entity edges that may
be instantiated in the graph. This allows unsubscription policies to be specified statically, using
only the application schema, while \sys ensures that any application entity graph instance satisfies
...
We next describe the choices for ghost entity generation and decorrelation policies that \sys makes
available.
...
\sys removes the parent entity and returns this data to the unsubscribing user. Note that any other
edge types from this parent to other children or grandparents may be cloned in the ghost entities,
so that \sys performs decorrelation only on this specific edge type.
...
Developers select from the following subpolicy options: 
    ...
    \textbf{Retain}: Nothing is done on either side of the
