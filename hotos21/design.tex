...
By default, \sys replaces all \emph{sensitive} entities---entities transitively connected via
parent-child edges to the mask's target entity---with ghost entities.
...
\end{enumerate}
\lyt{TODO: I think we need one more transformation type (or perhaps an additional argument type)
that says to Decorrelate or Delete \emph{only if} certain properties are satisfied. \eg, if the
parent user entity is from the EU and we need to have stricter privacy properties, etc.}
...
Data masks support these transformations on child-parent edge types, in which developers specify  a
