...
\sys encrypts the data with a per-user key, which is encrypted with a user password and stored by
the application. \sys then returns the encrypted data to the application. \lyt{Or \sys can manage
the storage of the data for the application?}
The user can optionally store this encrypted data themselves, and be in charge of providing their
key to DeCor to encrypt and decrypt the data.

\subsection{Resubscription}
Upon resubscription, the user allows \sys to decrypt the data by providing their password to decrypt
their key. \sys then uses the decrypted data to remove any created ghost entities, recorrelate entities back
