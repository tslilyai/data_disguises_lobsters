...
\paragraph{(1) CloneAll.} Ghosts generated from the same template all share the template's
    ...
    For example, Table~\ref{tab:ghosting} shows how the \texttt{darkmode} attribute is cloned in
    all ghosts; the \texttt{darkmode} attribute reveals very little about the underlying user's
    identity.

\paragraph{(2) Generate.}
...
\paragraph{(3) CloneOne.} One ghost entity shares the same value for the attribute as the
...
relationships (edges in the graph) to the mask's target entity---become ghost entities.
