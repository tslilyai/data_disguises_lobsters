...
Finally, the developer should decide how \sys generates ghost for all entities. 
...
Finally, the developer chooses to clone all paper conflicts. Paper conflicts have three attributes,
a value attribute for the conflict type (authorship or reviewer conflict), and two edge attributes
that specify a user parent and a paper parent respectively. It is safe to clone conflict types
because this reveals little identifying information, and does not interfere with HotCRP's execution.
As with paper entities, it is safe to clone edges to user and paper parents because any cloned edge
attributes will necessarily point to ghosts if there was a sensitive correlation between paper
conflicts and parent entity. 
