...
pub fn get_vault_cols() -> Vec<ColumnDef> {
    ...
    ColumnDef {
        name: Ident::new("modifiedObjectType"),
    ...
    // value that object was changed to (NULL if deleted)
    ColumnDef {
...
pub fn get_create_vault_statements(table_names: Vec<&str>, in_memory: bool) -> Vec<Statement> {
    let engine = Some(if in_memory { Engine::Memory } else { Engine::InnoDB });
