...
We evaluate whether \sys achieves the following goals:
\begin{itemize}
    \item Maintains normal execution query throughput for all users in the absence of disguise application
    \item Maintains normal execution query throughput for non-disguised users when one (or more)
        disguises are being applied
    \item Acceptable latency of (eventually consistent) disguise application
\end{itemize}
...
One \sys variant intercepts application queries, which decreases throughput in normal execution by
