...
\hline
    & \textbf{100B} & \textbf{1MB} & \textbf{10MB}\\
...
%
%
The cost of \sys's other cryptographic operations scales linearly with the
size of the stored data, but they are generally cheap (0.3--4.5ms).
%
...
The application and \sys may perform extra cryptographic work to apply a second disguise $d_2$ to
data decorrelated by a prior disguise $d_1$.
...
%
Upon reveal of $d_2$ with $p$'s private key, \lcapa{pd_1}, and the set of \lcapa{pd_2}s \sys decrypts and
...
Figure~\ref{f:composition} shows these costs of interactively applying account deletion to user $p$
after anonymization to further disguise $p$'s pseudoprincipals' data in WebSubmit and HotCRP, as
well as the cost of subsequently revealing $p$'s account with the returned locators.
As expected, restoration cost increases in absolute numbers substantionally more than deletion cost, since (with the
typical small bag data sizes) decryption is an order of magnitude more expensive than encryption.
WebSubmit account deletion and restoration latencies increase respectively by 5.5x and 5.7x; and in
HotCRP, latencies increase by 3x and 10x.
\lyt{TODO why?}

%
Had deletion been non-iteractively applied, locators for the $q_i$ would be encrypted and stored with
sys's metadata for $q_i$. This incurs an extra encryption cost per $q_i$ during $d_2$. However, the
initial cost of decrypting speaks-for records would be eliminated.
%
Revealing $d_2$ would simply take \lcapa{pd_1} and $p$'s private key (\S\ref{s:composition}); \sys
recursively retrieves $q_i$'s records by decrypting the encrypted \lcapa{q_id_2},
%with $q$'s private key (retrived from \lcapa{pd_1} with $p$'s private key).
which similarly incurs an additional decryption cost per $q_i$ disguised.
