...
also protects the correspondence of bags to natural principals' data.
%
Because $q$ is dissociated from the original natural principal $p$ that speaks for $q$
(possibly via intermediate principals), the attacker cannot determine if natural
principal $p$ ever existed, or if they have any data in \sys.
%

%
If an attacker compromises the server at time $t$, they can harvest the private keys
that a client provides for operations after $t$.
%
This lets the attacker reveal all data that \sys encrypted with this private key.
%
...
lectures with four questions each, and answers for each question for
each user (160k total answers).
%
This might correspond to all classes in a department, or to a large AI class.
%
We measure end-to-end latency, including request processing in WebSubmit,
\sys's operations, and the response to the client.
%
Since WebSubmit is a small application and the amount of data under disguise
is constant across users, we would expect low variance in the results.
%
...
%
Normal operations have comparable latencies with and without \sys, so \sys
adds little overhead due to \eg principal registration.
%
...
Record batching also has no effect in creating user accounts and editing unanonymized data
(as no records are involved).
%

WebSubmit users all have approximately the same amount of data (a set number of answers to
lecture questions). Thus, the spread in latencies is low. The simplicity of WebSubmit's
