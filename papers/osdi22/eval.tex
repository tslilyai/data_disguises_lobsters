...
pool of keypairs already requires 4.1MB.

After 10\% of users delete their accounts, \sys's storage requires on average 128MB. The cost is
...
produces $\approx$74--75k pseudoprincipals and requires 104--105MB; this dominates the overall metadata cost.  
...
%
In addition, each pseudoprincipal inserts a public-key into \sys's metadata, adding $\approx$0.25KB
per pseudoprincipal and 18-19MB combined.

The remaining $\approx$2MB come from diff records storing the removal of the user's account (about
1KB for the user DB entry, and 0.25KB for user's public key metadata). These originate from data
originally stored in the database (with 2MB being 0.8\% of the initial seeded Lobsters database
size of 261MB).
