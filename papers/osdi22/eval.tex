...
%
\sys ensures that only a client properly authenticated as a principal $p$ who is authorized to invoke $d$ can apply
(and later reveal) $d$.
%

\vspace{6pt}\noindent\textbf{\emph{(2) Secure Disguise Tokens.}}
%
\sys ensures that only an authenticated principal who provides decryption capability
...
%
\sys ensures that an attacker cannot learn the set of disguises that have disguised a principal's data.
%
...
We ran WebSubmit disguises on a WebSubmit database prepopulated with 2000 total users, 20 lectures
with 4 questions each, and answers for each question for each user.

We ran HotCRP disguises on a HotCRP database prepopulated with 450 total users (50 PC members), 450
papers (50 accepted), and 4 reviews and 4 comments per paper distributed evenly among PC members.  

We ran Lobsters disguises on a Lobsters database prepopulated with \lyt{TODO}, following the
