...
\end{tabular}
  \caption{\sys cryptographic operations are cheap (0.3--4.5ms), except for
    ...
    Encrypt/decrypt latency has a baseline cost of 0.3/2.8ms, which then linearly increases.} 
    \label{f:opstats}
\end{figure}
...
We measure the size of all data stored by \sys when some randomly selected 1.7k users (10\% of all users in Lobsters) have
deleted their accounts.
Prior to any account deletion, \sys's metadata (public keys of natural principals) and the in-memory
pool of keypairs already requires 0.3MB; after 10\% of users delete their accounts, \sys's metadata
requires in total 5.3--6.3MB of storage (2\% of the initial seeded Lobsters database size of 261MB).
