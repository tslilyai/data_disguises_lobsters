...
pseudoprincipals.
%
However, if a client speaking for the pseudoprincipals actively applies the disguise (\eg a user
...
records at some locator. If the call to store the record indicates an active client speaking-for $p$,
\sys stores the ciphertexts at \lcapa{pd}, and everything is fine (\lcapa{pd} is sent back to the
active client). 
%
If no active user is present, however, \sys stores encrypted records at some locator for $q$, \lcapa{qd}.
%
...
%
In the first case, $p$ was not active for $d_2$.
...
Since $p$ was not active during $d_2$, \sys stored a locator for $q_i$'s encrypted records
...
%
In the second case, $p$ had been active during $d_2$. \sys thus created a locator \lcapa{pd_2} for
