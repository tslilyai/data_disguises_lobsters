...
principal $q \neq p$, and who does not provide the relevant capability pairs \pcapa{pd_1}, 
...
\sys records the disguised state of data $x$ in each \tdata{pd} recording a update performed by $d_i$
...
\op{d_1} produces a global diff \tdata{pd_1}; \op{d_2} produces a normal,
encrypted and secured diff \tdata{pd_2}.

If \sys simply performs \op{d_2}'s update to $x$, an adversary can still read undisguised data
from the \tdata{pd_1} diff because the diff is global and records a now-stale state of $x$!
%
To prevent global diffs like \tdata{pd_1} from leaking data that should be disguised, \sys
updates the data in \tdata{pd_1} with \op{d_2}'s update. An update to a diff such as
\tdata{pd_1} itself generates a (private) diff \tdata{pd_2}, just as updating a data
object generates a diff.

Revealing $d_2$ uses \tdata{pd_2} to reveal the modification to \tdata{pd_1}. 
If $d_1$ has already been revealed, and the database state of $x$ updated to reflect the
contents of \tdata{pd_1} (which were updated by the application of $d_2$), then \sys
simply uses \tdata{pd_2} to reveal the current state of $x$ in the database. 
...
diff \tdata{pd_2} stored in \lcapa{pd_2} that saves the removal of \tdata{pd_1} for reversal.}
