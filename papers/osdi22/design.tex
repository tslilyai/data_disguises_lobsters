...
locator \lcapa{pd} that will allow \sys to later locate the (encrypted) diff and
...
supplies $p$'s private key and the locator \lcapa{pd} that WebSubmit emailed to
...
%
WebSubmit receives the key and the locator, and invokes \fn{DiffsForDisguise} with
...
application database to its original state. Disguised data in \sys is no longer needed: WebSubmit
invokes \fn{ForgetData} with the locator and the delete-account disguise ID $d$, which clears all
trace of stored records from $d$.
%
...
%
The application would then remove these pseudoprincipals, and invoke \fn{ForgetPrincipal} on each
pseudoprincipal ID to remove \sys's state for the pseudoprincipal (subject to conditions described
...
private key and the locator \lcapa{pd} emailed by WebSubmit when it applied
...
private key and the locator \lcapa{pd} that WebSubmit sent when it applied the
