...
invokes \fn{StoreDiff(}$p$, $d$, \fn{buffer), \texttt{Some}($p$))}.
%
...
%
\item invokes \fn{StoreSpeaksFor(}$p$, $p_{\texttt{anon}}$, $d$, \texttt{None}\fn{)} to tell
...
%
This informs \sys to complete any record encryption operations and return locators \lcapa{pd} that
will allow \sys to later locate the (encrypted) diff and speaks-for records stored during disguise
$d$.
%\lyt{All records stored with $p$ as the active principal are encrypted and stored at $p$'s locator;
%otherwise, \sys stores records stored with no active user at the owning principal's locator.}
%
...
%
WebSubmit emails all locators \lcapa{pd} where $p$ is a natural principal to the appropriate client,
...
previously-deleted account; \two{} a user views or edits their answers after the
...
supplies $p$'s private key and any locators \lcapa{pd} that WebSubmit emailed to
the client when it applied the account deletion disguise $d$.
%\lyt{The locator is actually a list because of composition: the client may introduce locators from
%previous anonymization disguises so it can reveal data from disguise $d$ on pseudoprincipals. Not
%sure if this is too complex for here, or where to put this in.}
%
WebSubmit receives the key and the locators, and invokes \fn{DiffsForDisguise} with
each \lcapa{pd} and the private key.
%

\sys uses the private key to decrypt the records stored earlier, and returns the
(plaintext) diffs, which WebSubmit then uses to return the application database
to its original state.
%
Disguised data in \sys is no longer needed: WebSubmit invokes \fn{ForgetData} with
...
answers rather than decorrelating them.
%
WebSubmit thus only handles diff records, but if the disguise had decorrelated
data, the application would invoke \fn{PseudoprincipalsOf} with the \lcapa{pd}
...
\fn{ForgetPrincipal} on each pseudoprincipal ID to remove \sys's state for the
pseudoprincipal (with some caveats described in \S\ref{s:composition}).
%(subject to conditions described
%in \ref{s:composition}) \lyt{I don't love this forward reference, but we need this condition to be
%accurate}.

\head{(2) Accessing Anonymized Answers.}
%
When a client chooses to edit their anonymized answer, the client provides $p$'s
private key and locators \lcapa{pd} emailed by WebSubmit when it applied
the anonymization disguise $d$.

When WebSubmit receives the request, it invokes \fn{PseudoprincipalsOf} with each
\lcapa{pd} and private key to determine which pseudoprincipals $p$ may speak-for.
%
Once WebSubmit confirms that $p$ may speak-for the pseudoprincipal associated
(in the application DB) with the lecture answers to be edited,
...
private key and locators \lcapa{pd} that WebSubmit sent when it applied the
anonymization disguise $d$.
%
WebSubmit then invokes \fn{PseudoprincipalsOf} with each \lcapa{pd} and the private
...
%
(\sys continues to remember the pseudoprincipals, however, so the user
...
modifications: edit row, delete row, and decorrelate along foreign key.
%
...
%
Finally, \fn{ApplyDisguise} takes an optional private key and locator set
to support disguise composition (\eg deletion after anonymization).
%
