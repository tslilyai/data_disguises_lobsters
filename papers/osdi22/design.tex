...
invoking \fn{RegisterPrincipal} with the new principal's UID $p$ (the value of the \fn{id} column in the \fn{users} table).
\sys generates a public-private key pair for $p$, returning private key \privk{p} to 
WebSubmit, and saving the public key \pubk{p}.
...
\privk{p} emailed to it upon $p$'s registration, and the locators \lcapa{pd} emailed to it by WebSubmit when account deletion disguise $d$ was applied to $p$. 
