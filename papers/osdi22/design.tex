...
invokes \fn{StoreDiff(}$p$, $d$, \fn{buffer), \texttt{Some}($p$))}.  Setting \fn{activeP} to
...
\item invokes \fn{StoreSpeaksFor(}$p$, $p_{\texttt{anon}}$, $d$, \texttt{None}\fn{)} to tell
    \sys to securely store the speaks-for link between $p$ and $p_{\texttt{anon}}$. Setting
        \fn{activeP} to \texttt{None} indicates to \sys that the disguise is performed by the
...
\lyt{These private keys are actually per-pseudoprincipal, and so WebSubmit needs to invoke
\fn{DiffsForDisguise} with the right private key. This feels like too complex to put here?}
%
