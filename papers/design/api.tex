...
\hline
    ...
    \fn{ApplyDisguise($p$, DisguiseSpec $d$,
        ...
        disguise $d$ and returns the corresponding disguise instance $\delta_i$ and 
        capability pair \pcapa{p\delta_i}. The provided capability pairs allow \sys to locate and access
...
\textbf{API Call} & \textbf{Description} \\
\hline
    \fn{CapPairEstablishesOwnership($p$, DatabaseObj $x$, CapPair \pcapa{p\delta_i})}
...
during application. Applying a disguise returns the instance identifier $\delta_i$ and the locating
and data capability pair \pcapa{p\delta_i} that only the client stores. 
...
\fn{CapPairEstablishesOwnership($p$, $x$, \pcapa{p\delta_i})} for all capability pairs provided by the
client.
