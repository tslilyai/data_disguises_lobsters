...
\textbf{API Call} & \textbf{Description} \\
\hline
\fn{RegisterPrincipal($p$, Email email, PubKey $\pubk{p}$)} $\rightarrow$ () & Wrapper to \sys's
...
shown in Table~\ref{tab:app_api}, such that the client can pass capabilities to the application
along with the action to perform.
...
The client provides the capability pair \pcapa{p}{d}. The application passes these to
