...
\hline
    \fn{ApplyDisguise(PrincipalID $p$, DisguiseSpec $d$,
        Set<(LocCap, DataCap)> caps)} $\rightarrow$\fn{(DisguiseInstance, Set<LocCap>)} & Applies
    ...
    prior disguises.
        \vspace{6pt}\\
    \fn{RevealDisguise(PrincipalID $p$, DisguiseInstance $\delta_i$,
        Set<(LocCap, DataCap)> caps)} $\rightarrow$ ()& 
    ...
    diffs from applying $\delta_i$, which allows \sys to reveal undisguised data.
        \vspace{6pt}\\
    \fn{GetDataCap(PrincipalID $p$, DisguiseInstance $\delta_i$,
...
\textbf{API Call} & \textbf{Description} \\
\hline
    \fn{RegisterPrincipal(PrincipalID $p$, Email email, PubKey $\pubk{p}$)} $\rightarrow$ () & Registers $p$ as
        a principal.\\
    \vspace{6pt}\\
    \fn{CapEstablishesLink(PrincipalID $p$, DatabaseObj $o$, LocCap \lcapa{p\delta_i}, DataCap
    \dcapa{p\delta_i})}
    ...
    we want to assume.}
    \vspace{6pt}\\
    \fn{CapToDiffs}\fn{(LocCap \lcapa{p\delta_i}, DataCap \dcapa{p\delta_i})} $\rightarrow$ \fn{Set<Diff>} & Returns the
...
\head{Retrieving Capabilities.}
Applying a disguise returns the instance identifier $\delta_i$ and a locating
capability \lcapa{p\delta_i} that the client remembers. 
%
However, clients need to invoke disguise application or reveal with capability \emph{pairs}, not just
locating capabilities.
This is because the locating capability enables access to the encrypted data
capability \dcapa{p\delta_i} and encrypted disguise diffs \tdata{p\delta_i}, while
the data capability \dcapa{p\delta_i} enables decryption of the encrypted \tdata{p\delta_i}. 

Client therefore must retrieve \dcapa{p\delta_i} before invoking the disguise or reveal API calls.
A client who has \lcapa{p\delta_i} and \privk{p} can get a capability pair for $p$ and $\delta_i$ by
calling \fn{GetDataCap} to acquire the encrypted \dcapa{p\delta_i}. The client can then decrypt it
with \privk{p} to get \dcapa{p\delta{i}}. 
%
Then the client has the necessary (\lcapa{p\delta_i},\dcapa{p\delta{i}}) capability pair to provide to future disguise or reveal invocations, and enable
access to \tdata{p\delta_i}.
...
spec, and a set of locating and data capability pairs. 
These capability pairs allow \sys to access disguise diffs of the corresponding disguises to
compose the new disguise on top of these disguises during application.

In order to reveal data disguised by $\delta$, a client speaking for $p$ invokes
\fn{RevealDisguise}.
The client provides the capability pair (\lcapa{p\delta_i},\dcapa{p\delta{i}}) to allow \sys to
access disguise diffs that it can use to
restore the original, undisguised data.

\head{Granting Post-Disguise Permissions.}
\lyt{TODO, also need to add how flow of client getting capability to hand to application via e.g., a
url would work.}
...
query \fn{IsCorrelated($p$, paper|review, \dcapa{p\delta})}, where \dcapa{p\delta} is provided by the
