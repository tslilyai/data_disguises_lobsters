...
\hline
    \fn{ApplyDisguise($p$, Disguise $d$, Vec<CapPair> caps)} $\rightarrow$\fn{Vec<CapPair>} & 
        Applies disguise $d$ and returns the corresponding disguise $d$ and capability pair
        \pcapa{p}{d}. The provided capability pairs allow \sys to locate and access disguise diffs,
        which \sys uses to compose disguise $d$ on top of prior disguises.
        \vspace{6pt}\\
    \fn{RevealDisguise($p$, Disguise $d$,
        Vec<CapPair> caps)} $\rightarrow$ ()& 
    Reveals disguise $d$. The provided capability pairs grant \sys access to database
    diffs from applying $d$, which allows \sys to reveal undisguised data.
        \vspace{6pt}\\
   \fn{CapPairEstablishesOwnership($p$, DatabaseObj $x$, CapPair \pcapa{p}{d})}
    $\rightarrow$ \fn{bool} & Returns
    whether principal $p$ has ownership rights to the provided database object. The capability pair
    acts as proof that $p$ had been correlated with the object. 
    %\fn{CapPairToDiffs}(\fn{CapPair \pcapa{p}{d}}) $\rightarrow$ \fn{Vec<Diff>} & Returns the
...
\textbf{API Call} & \textbf{Description} \\
\hline
    \fn{ApplyDisguise($p$, Disguise $d$,
        Vec<CapPair> caps)} $\rightarrow$\fn{Vec<CapPair>} & 
        Wrapper around a call to \sys's \fn{ApplyDisguise}.
        \vspace{6pt}\\
    \fn{RevealDisguise($p$, Disguise $d$,
        Vec<CapPair> caps)} $\rightarrow$ ()& 
        Wrapper around a call to \sys's \fn{RevealDisguise}.
        \vspace{6pt}\\
\fn{ActWithCaps($p$, Action $a$, Vec<CapPair> caps)} $\rightarrow$ bool & Requests
    a particular application action $a$ specific to the application's API (\eg read access to paper reviews) with the provided
    capabilities.
\end{tabular}
\caption{Modified Application API}
\label{tab:app_api}
\end{table*}
...
revealing.
%
The application also uses \sys to gain access to data or information granted by the provided capabilities. 
Applications integrating with \sys may modify the application API invoked by clients, shown in
Table~\ref{tab:app_api}, such that the client can pass capabilities to the application along with
...
A client speaking for principal $p$ applies a disguise by invoking the application wrapper around
\fn{ApplyDisguise}; the application invokes \sys's library call, 
with the provided disguise and a set of locating and data capability pairs. These capability pairs
allow \sys to access disguise diffs of the corresponding disguises to compose the new disguise on
top of these disguises during application. Applying a disguise returns the locating and data
capability pair \pcapa{p}{d} that only the client stores. 

In order to reveal data disguised by $d$, a client speaking for $p$ invokes \fn{RevealDisguise}.
The client provides the capability pair (\lcapa{pd},\dcapa{p}). The application passes these to
\sys to restore the original, undisguised data with access to the corresponding diffs.
...
authorship or reviewership permissions, the client first invokes the \fn{ActWithCaps} API call to
...
\fn{CapPairEstablishesOwnership($p$, $x$, \pcapa{p}{d})} call for all capability pairs provided by
the client.

If any call returns \fn{true}, the application can establish $p$'s ownership of $x$, even if $x$ is
...
%However, clients only ever receive and store locating capabilities \lcapa{pd} returned from
%the application of $d$.
%%
%Thus, \sys provides a way for a client who has \lcapa{pd} to acquire the full capability pair
%by retrieving an encrypted \dcapa{p} via \fn{GetEncryptedDataCap} before invoking API calls.
%
%\sys stores \dcapa{p} encrypted with \pubk{p}, so only a client who has both
%\lcapa{pd} and \privk{p} can decrypt the respond with \privk{p} to get the necessary
%\pcapa{p}{d} capability pair. 
