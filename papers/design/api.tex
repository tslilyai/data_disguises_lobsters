...
\textbf{API Call} & \textbf{Description} \\
\hline
\fn{RegisterPrincipal($p$, Email email, PubKey $\pubk{p}$)} $\rightarrow$ () & Wrapper around \sys's
        \fn{RegisterPrincipal}.\\
    \vspace{6pt}\\
    \fn{ApplyDisguise($p$, Disguise $d$,
        Vec<CapPair> caps)} $\rightarrow$\fn{Vec<CapPair>} & 
        Wrapper around \sys's \fn{ApplyDisguise}.
        \vspace{6pt}\\
    \fn{RevealDisguise($p$, Disguise $d$,
        Vec<CapPair> caps)} $\rightarrow$ ()& 
        Wrapper around \sys's \fn{RevealDisguise}.
        \vspace{6pt}\\
