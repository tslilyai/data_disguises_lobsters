...
\textbf{API Call} & \textbf{Description} \\
\hline
\fn{ActWithCapabilities($p$, Action $a$, Vec<CapPair> caps)} $\rightarrow$ bool & Requests
...
\textbf{API Call} & \textbf{Description} \\
\hline
    \fn{CapPairEstablishesLink($p$, DatabaseObj $o$, CapPair \pcapa{p\delta_i})}
...
\pcapa{p\delta_i},\dcapa{p\delta{i}} capability pair. 
...
\head{Client-Application-\sys: Performing Permitted Application Actions.}
...
If the client speaking for $p$ invokes application action $a$, such as ``edit review'', as normal, the application
...
principal $p$ is linked to object $o$ (\eg $p$ is the author of some paper or review). To work with
disguises, the application developer augments these these checks with a queries to \sys of
\fn{CapPairEstablishesLink($p$, $o$, \pcapa{p\delta_i})} for all capability pairs provided by the
client.

If any query returns \fn{true}, the application can establish a link between $p$ and $o$, even if $o$
