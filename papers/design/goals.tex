...
\head{Example in HotCRP.}
We imagine three useful disguises to support in HotCRP: (1) per-user GDPR account deletion (which
removes all of a particular user's data); (2) universal account pseudonymization (which modifies a user's
account to remove any personal identifiers); and (3) universal decorrelation (which decorrelates users from
...
We generalize from the HorCRP example a handful of the most beneficial and practical use cases
...
\begin{itemize}
	...
	We denote an application principal as $p \in P$, where $P$ is the set of all active and
        inactive application principals.\lyt{Wait why do we have a distinction between active and
        inactive here?}
	%
...
ever accesses and needs to parse diff contents directly: \sys exposes an API allowing the client or
application to learn specific semantic information contained in diffs (\eg whether a particular
principal used to own a now-decorrelated piece of data). Clients can additionally request
that \sys access and use diff information to reveal or apply disguises.

\sys's access to diffs is controlled by a pair of capabilities: 
\begin{enumerate}
