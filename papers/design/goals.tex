...
\tdata{p\delta_i} & database diff that stores the undisguised data associated with $p$ prior to application of $d$.\\
    \vspace{6pt}
\dcapa{p\delta_i} & data capability that grants access to database diffs from $\delta_i$, as well as
    pre-$\delta_i$ correlations. Data capabilities are associated with a principal $p$.\\
    \vspace{6pt}
\lcapa{p\delta_i} & locating capability that allows the holder to find the database diffs from
    $\delta_i$. Locating capabilities are associated with a principal $p$.\\
    \vspace{6pt}
...
%
\vspace{6pt}\noindent\textbf{\emph{(1) Authenticated and Authorized Disguises.}}
%
Only a client properly authenticated as a principal $p$ who is allowed to invoke $d$ can apply (and
later reveal) $\delta_i$.
\lyt{This was ``authorized disguise'' previously; but we mean authenticated?}
%

%
\vspace{6pt}\noindent\textbf{\emph{(2) Secure Linking.}}
%
Only a client who has the capability \dcapa{p\delta_i} can relink $p$
...
%
Only a client who has the capability \dcapa{p\delta} can grant \sys access to the disguise changes from 
disguise instance $\delta_i$.
%
...
\begin{itemize}
	...
	authorized client provides \dcapa{p\delta_i}. \ms{as above}
    ...
    \item Privacy guarantees about undisguised data: if identifying data about $p$ is not covered by
        disguise specification $d$, it remains visible to attackers after $\delta_i$ applies.
