...
\item\textbf{Temporary Recorrelation of Decorrelated Data for Disguise Composition.}
A disguise targeting a user's data can, if authorized, also operate on data decorrelated from the user by
prior disguises.
For example, GDPR account deletion removes user $u$'s reviews even if \sys has decorrelated these
        reviews from $u$.

\item\textbf{Temporary Recorrelation of Decorrelated Data for Per-User Revealed Views and Permissions.}
Users should be able to operate with ownership permissions on data previously correlated with their
account, but decorrelated by a disguise, \emph{without} changing the database contents.
In other words, users can see a personalized, temporary database view that allows them to operate on
previously owned data.

    For example, a user whose authored papers have been decorrelated can still log in and view their
