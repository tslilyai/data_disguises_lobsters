...
disguised data, by logging in and clicking one url link.

\head{Per-User, GDPR-Compliant Disguising and Revealing.}
...
\head{Universal Disguising and Revealing.}
\sys should support universal, administrator-applied disguise to decorrelate and anonymize all users in the system.
...
\head{Per-User Revealed Views and Permissions.}
...
\head{Further Disguising of Disguised Data.}
...
To support these use cases, applications work with two abstractions of \sys:
\begin{itemize}
...
The application's normal permissions logic controls data access to database contents. 
Clients and the application using \sys never directly access diff contents: instead, \sys exposes
an API allowing the client or application to learn specific information contained in the diff.
Clients can additionally that request \sys access and use diff information to reveal or apply
disguises.

These API calls require \sys to access diffs, and \sys's access to diffs is controlled by a pair of 
capabilities: 
\begin{enumerate}
...
\sys's design intends for Applications 
