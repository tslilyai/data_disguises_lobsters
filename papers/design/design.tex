...
\< \sendmessageright*{\fn{RegisterPrincipal(p,email,$\pubk{p}$)}} \< \\
\<\< \fn{StorePubKey($\pubk{p}$)}\\
    \{\capa{pd'}\} \gets \fn{LoadPriorDisguiseCapabilities()} \<\< \\
    \< \sendmessageright*{\fn{CapabilitiesToEncKeys(\{\capa{pd'}\})}} \< \\
    \<\< \fn{LoadEncKeys(\{\capa{pd'}\})}\\
    \< \sendmessageleft*{\{Enc(\symk{pd'})\}} \< \\
\fn{symKeys} \gets \{\}\\
\pcforeach \fn{Enc(\symk{pd'})} \in \fn{\{Enc(\symk{pd'})\}}:\\
    \quad \fn{symKeys.insert(\dec(\privk{q}, \symk{pd'}))}\\
\pcendforeach\\
    \< \sendmessageright*{\fn{Disguise($d$,symKeys)}} \< \\
    \<\< \fn{ApplyDisguise($d$,symKeys)}\\
\< \sendmessageleft*{\{\capa{pd}\}} \<\\
\fn{StoreCapabilities(\{\capa{pd}\})} \<\< \\
}
...
application. 
An application can apply a disguise $d$ (\sys returns \capa{pd}s for the produced disguise tokens); 
...
\head{External Storage of \capa{pd}.} \sys should not store \capa{pd} such that \sys can learn the
...
\sys can publically save a mapping from $p$ to \capa{pd}; this leaks disguise-principal associations, but
