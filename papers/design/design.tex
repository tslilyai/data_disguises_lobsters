...
key \texttt{pk}$_p$ along with $p$'s ID.
}
\label{fig:acctreg}
\end{figure*}
...
\texttt{deck\_set} \gets \{\}\\
\forall d'~\text{that the client authorizes token access}: \\
    \quad\texttt{deck\_set.insert}(\dec(\privk_c, \texttt{encks}[d'])) \<\< \\
    \< \sendmessageright*{\texttt{GrantTokenAccess(\texttt{deck\_set})}} \< \\
...
\<\< \forall d'~\text{previously applied}: \\
    ...
    \<\< \forall \key_{pd'} \in \texttt{deck\_set}: \\
    \<\< \quad\texttt{tokens.insert(\fn{ReadPrivateTokens($\key_{pd'}$)})}\\ 
\<\< \texttt{ApplyDisguise($d$,tokens)}
}
\caption{When a client authenticated as some principal $p$ wishes to apply a disguise $d$, the
client invokes the \fn{StartDisguise} API call. 
