...
\<\< \texttt{store(enck$_{cd}$)} \pclb
\pcintertext [dotted]{FOR ALL PAST DISGUISES $d'$:} \\
\<\< \\[-3\baselineskip]\\
\<\< \texttt{enck}_{cd'} \gets \texttt{RetrieveEncKey}(c,d')\\
\< \sendmessageleft*{\texttt{AuthorizeTokenAccessRequest(enck}_{cd'})} \< \\
\key_{cd'} \gets \dec(\sk_c, \texttt{enck}_{cd'}) \<\< \\
...
\<\< \\[-3\baselineskip]\\
\<\< \texttt{ApplyDisguise($c,d$,tokens,$\key_{cd}$)}
}
...
\begin{figure*}
\procb{\texttt{ReadTokens}$(c,d,k_{cd})$}{
hello
}{}

\procb{\texttt{ApplyDisguise}$(c,d,tokens,k_{cd})$}{
hello
}{}

\procb{\texttt{ReverseDisguise}$(c,d,tokens)$}{
hello
}{}

\caption{Edna Algorithms}
\end{figure*}
