...
\<\< \texttt{tokens.append(ReadTokens(keys))} \\
...
\texttt{keys}[d'] \gets \dec(\sk_c, \texttt{encks}[d']) \<\< \\
\< \sendmessageright*{\texttt{AuthorizeTokenAccessReply(keys)}} \< \\
\<\< \texttt{tokens.append(ReadTokens}(\texttt{keys})) \\
...
\procb{\texttt{ApplyDisguise}$(c,d$,\texttt{tokens},$\key_{cd})$}{
hello
}{}
