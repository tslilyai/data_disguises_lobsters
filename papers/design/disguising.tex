...
%D1 tokens inaccessible --> D2 can be anything --> Rev D1 requires applying D2 updates
%
%D1 tokens accessible --> D2 can be anything, UPDATES TOKENS --> Rev D1 simply restores D1 token data
%
...
%
%%(Global?) tokens aren't restored to undisguised state if new state doesn't match recorded in
%disguise token --> means that another disguise has been applied (similar approach to app updates)

\sys must support \emph{delayed disguise actions} and \emph{token modification} in order to
achieve the end states described in Table~\ref{tab:composeapprev1}. 
%Without these techniques,
%reversal of $d_1$ in history [\app{d_1},\app{d_2},\rev{d_1}] would always result in
%state \ohist{[\app{d_1},\app{d_2}]}.

\noindent\textbf{\emph{Delayed Disguise Actions.}}
Consider the history [\app{d_1},\app{d_2}].
...
However, \sys needs to ensure that \rev{d_1} achieves the semantics described in
...
% NOTE this is an issue in a history that does rev d2 after rev d1, not the ones shown above..
