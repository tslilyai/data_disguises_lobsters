...
%
While Table~\ref{tab:composeapprev1} shows the possible outcomes of
\ohist{[\app{d_1},\app{d_2},\rev{d_1}]}, this is a necessary separate scenario to consider because
$d_2$ may have updated $O$ in a way that conflicted with $d_1$, so \rev{d_1} could not actually
revert the update performed by $d_1$ to $O$.
%
In order to revert this update, the client invoking \rev{d_2} must have access to $d_1$'s tokens for
