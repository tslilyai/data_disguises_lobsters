...
\caption{Possibilities for \xhist{[\app{d_1},\app{d_2}]} depending on the state of the client invoking $d_2$, and the
...
\end{tabular}
\vspace{6pt}
\caption{\xhist{[\app{d},\rev{d}]} depending on the state of the client invoking
\rev{d}, and the state of tokens from $d$ regarding $x$.}
...
\end{tabular}
\vspace{6pt}
\caption{\xhist{[\app{d_1},\app{d_2},\rev{d_1}]} depending on the state of the client invoking
...
depends on which client invokes \rev{d_1}, and how $d_1$'s tokens for $x$ are stored. Similar to
