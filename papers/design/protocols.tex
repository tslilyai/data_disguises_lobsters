...
\hline
    ...
    set of addresses corresponding to the addresses of \tdata{} and \tokk{} ciphertexts produced by this disguise.\\
    \vspace{6pt}
    \fn{Reveal($d$, \{\tokk{pd}\}) $\rightarrow ()$} & 
...
\< \sendmessageright*{\fn{AddressesToEncTokenKeys(addrs)}} \< \\
    \<\< \fn{encTokKeys} \gets \fn{LoadEncTokenKeys(addrs)}\\
\< \sendmessageleft*{\fn{encTokKeys}}\\
\fn{tokkeys} \gets \{\}\\
\pcforeach \fn{Enc(\tokk{qd'})} \in \fn{encTokKeys}\\
\quad \pcif \text{authorizes access to $q$'s private tokens for $d'$}: \\
\quad\quad \privk{q} \gets \fn{privKs[q]}\\
\quad\quad \fn{tokkeys.insert(\dec(\privk{q}, \tokk{qd'}))}\\
\pcendforeach\pclb
\pcintertext[dotted]{Apply disguise with token keys}
\< \sendmessageright*{\fn{Disguise($d$,tokkeys)}} \< \\
\<\< \{\addr{}\}\gets\fn{ApplyDisguise($d$,tokkeys)} \pclb
...
Using these addresses, the client can query for encrypted symmetric \tokk{} keys corresponding to
the set of provided addresses; and decrypt and provide these \tokk{}s to \sys to disguise (or reveal) using token data that can be decrypted using the provided \tokk{}.
...
provides the decrypted \tokk{pd} and \sys tries to decrypt every ciphertext in the bag. This clearly
...
\hline
    ...
    \privk{p} and uncover the corresponding \tokk{pd}s. \\
    \vspace{6pt}
    \fn{GrantTokenAccess(\{\tokk{pd}\})} & Grants \sys permission, for all \tokk{pd} in the
    argument, to decrypt any privately-stored \tdata{pd} and \tpriv{pdq} with \tokk{pd}.
\end{tabular}
    \vspace{6px}
...
provided set, and retrieves the corresponding encrypted \tokk{}s.
\sys then requests to access all private tokens protected by these symmetric keys by sending a
\fn{RequestTokenAccess} to the client with the \tokk{} ciphertexts.
...
%
The client only includes the decrypted \tokk{qd'} in the set of keys sent back to \sys via a
\fn{GrantTokenAccess} API call if the client wants to authorizes access to $q$'s private tokens
from disguise $d'$. 

\sys then uses each \tokk{qd'} to decrypt $q$'s private tokens produced by disguise $d'$. Note that
if the client returns a false \tokk{qd'}, the decryption will fail, and \sys will not reveal any
...
\<\< \pcendforeach\\
\<\< \pcforeach \tokk{qd'} \in \fn{symKeys}: \\
\<\< \quad\fn{tokens.insert(}\\
\<\< \quad \quad \fn{ReadPrivateTokens(\tokk{qd'}))}\\ 
\<\< \pcendforeach\\
