...
\<\< \texttt{ApplyDisguise($d$,tokens)}
    ...
    Prior to applying the disguise, \sys requests to access tokens owned by the client by
...
The client sends all decrypted $\key_{pd'}$s back to \sys via a \fn{GrantTokenAccess} API call. 
\sys uses each $\key_{pd'}$ to decrypt $p$'s private tokens produced by disguise $d'$,
