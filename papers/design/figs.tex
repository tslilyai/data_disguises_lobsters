...
\<\< \fn{privKeyEncs} \gets \fn{LoadEncPrivKeyTokens}(p)\\
\< \sendmessageleft*{\fn{privKeyEncs}} \< \\
\texttt{privKeys} \gets \{p \mapsto \privk{p}\}\\
\texttt{principalIds} \gets \{p\}\\
% TODO recursively loop to get all privkey tokens for all principals
\pcforeach \fn{privKey} \in \fn{privKeyEncs}:\\
\quad \tpriv{pd'q} \gets \dec(\privk{p}, \fn{privKey})\\
\quad \pcif \text{authorizes access to $q$'s private tokens}: \\
\quad\quad\texttt{privKeys.insert}(q\mapsto \tpriv{pd'q}.\texttt{privKey}) \<\< \\
\quad\quad\texttt{principalIds.insert(q)} \<\< \\
\quad\pcfi\\
\pcendforeach\\
\< \sendmessageright*{\fn{RequestEncKeys($d$,principalIds)}} \< \\
\<\< \texttt{symKeyEncs} \gets \{\}\\
\<\< \pcforeach q \in \fn{principalIds}:\\
\<\< \quad\fn{symKeyEncs.insert(LoadEncSymKeys}(q))\\
\<\< \pcendforeach\\
...
\quad \symk{qd'} \gets \dec(\privk{q}, \fn{encSymKey})\\
\quad \pcif \text{authorizes access to $d'$-produced tokens for $q$}: \\
\quad\quad\texttt{symKeys.insert(\symk{qd'})} \<\< \\
\quad\pcfi\\
\pcendforeach\\
...
\<\< \quad\texttt{tokens.insert(\fn{ReadGlobalTokens}($d'$))}\\ 
\<\< \pcendforeach\\
\<\< \pcforeach \symk{qd'} \in \texttt{symKeys}: \\
\<\< \quad\texttt{tokens.insert(\fn{ReadPrivateTokens(\symk{qd'})})}\\ 
\<\< \pcendforeach\\
\<\< \texttt{ApplyDisguise($d$,tokens)}
}
