...
However, we now have a problem. Diffs are encrypted with \dcapa{p} ($p$'s private key) and stored at
...
\end{enumerate}
This ensures that a client with \lcapa{pd} and \dcapa{p} (\privk{p}) can access \privk{q}, the data
...
\begin{enumerate}
    \item stores disguise diffs \tdata{qd'} at location \lcapa{qd}
