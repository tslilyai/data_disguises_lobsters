...
%
In some situations, reversible disguises are desirable.
%
For example, a user might choose to return to the applicaton, or a disguise may
modify the same data already modified by a prior disguise.
%
Many disguises edit the physical database contents in (often necessarily)
information-destroying ways, so the disguising framework includes \emph{per-user
vaults} that allow reversing some disguises.
%
These vaults are individually owned storage shards that log all disguises applied
to an individual user's data.
%
...
user approval.
%
On disguise reversal, the disguising tool accesses the relevant user vaults to obtain the
original, unmodified data.
%

%
We believe that flexible disguising is importy to meet applications' diverse privacy
requirements, and that better user privacy on the web is best achieved through a
systematic approach that reduces developer burden.
%
