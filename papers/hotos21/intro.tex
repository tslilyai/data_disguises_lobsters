...
%
Beyond legal mandates, it is good practice to minimize retained user data.
%

%
Consequently, application developers increasingly face the need to implement
...
%
As another example, a system like HotCRP~\cite{hotcrp} could anonymize old peer review data to avoid having a future data breach expose reviewer identities.
%
...
%
Because many disguises edit the physical database contents in (often necessarily)
information-destroying ways, the disguising framework includes \emph{per-user vaults}, namely
individually owned storage shards that log all disguises applied to an individual user's data.
Vaults allow for selective disguise reversal, which may be triggered explicitly (\eg by a user who
wants to return to the application), or by necessity, in order to correctly apply a disguise that
