...
When applying a disguise, a tool must both correctly apply the specified transformations, and also
...
To handle inter-disguise dependencies, a tool relies on (1) the structured nature of disguises to
...
disguises by providing a secure way to store the data. A tool queries the user vault to
...
%
The vault entries could be configured to expire after a certain time (requiring that a tool either
...
%
In particular, a tool ensures that prior disguises do not affect \emph{which} objects are updated
...
A tool allows developers to reason about multiple conflicting updates to the same object by ensuring that
if one disguise removes an object that another disguise modifies, the removal takes
precedence.
%
However, if two disguises modify the same object attribute, a tool
...
A disguising tool applies disguises in a four-phase procedure:
\begin{enumerate}[nosep]
    \item \emph{Prepare}: reconcile any data dependencies between this disguise and prior disguises.
            A tool detects read-after-write dependencies between the new disguise's predicates and prior disguises'
    ...
    \item \emph{Record}: store records of all updates in the appropriate per-user vaults. A tool
...
it missed a subsequent disguise, A tool keeps a persistent log of all disguises performed by the
